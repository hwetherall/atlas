...
// ─────────────────────────────────────────────────────────────────────────────
// FROZEN SNAPSHOT — ledger rev 1 (git 2428e977), the model BEFORE refinement
// cycle 1. The demo's "before" world: the initial fact bank, dashboard and
// risk register render from this; the live lib/ledger.ts is rev 2 (corrected).
// Never curate this file — it is history.
//
...
const rawLedger: FactNode[] = [
  // ── tamBase — the scope-down anchor / top value-of-information ─────────────
  {
    ...
    derivation: {
      plain:
        "The world buys about $2.8B of rack PDUs a year. Europe's slice is roughly 30% of that, and the seven Central European countries in scope hold about 40% of Europe's data-center capacity — multiply through and the Central European market comes out near €310M, which we carry as €320M. A cruder method using a broader power-equipment category gives €495M; that sets the top of the uncertainty band, not the estimate.",
  ...
  },
  {
    ...
    derivation: {
      plain:
        "Companies running their own server rooms account for 32% of capacity in the industry split; after the edge and telecom carve-out, that is 29% of the market.",
      method: "Capacity-split rescaling",
  ...
  },
  {
    ...
    derivation: {
      plain:
        "Of the market we can reach, how much can be won in the first year? Startup benchmarks put first-year capture at 1–5% of the serviceable market; we carry 3% as the middle. The earlier 6% sat above every benchmark found, so it had already been revised down once.",
...
export const ledgerRev1: Ledger = validateLedger(rawLedger);

// Every skillId on a node must resolve to a real skill — throws at boot on a typo.
validateSkillRefs(ledgerRev1);
